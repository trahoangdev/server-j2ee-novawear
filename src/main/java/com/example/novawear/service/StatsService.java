...
import com.example.novawear.repository.OrderDetailRepository;
...
public class StatsService {
    ...
    public List<TopProductDto> getTopSellingProducts(Instant from, Instant to, int limit) {
        List<Order.OrderStatus> completed = List.of(Order.OrderStatus.DELIVERED, Order.OrderStatus.SHIPPED,
                Order.OrderStatus.CONFIRMED, Order.OrderStatus.PROCESSING);
        return orderDetailRepository.findTopSellingProducts(completed, from, to, PageRequest.of(0, limit));
    }
    ...
    public RevenueStatsDto getRevenueStats(Instant from, Instant to) {
        ...
        for (LocalDate d = from.atZone(ZoneOffset.UTC).toLocalDate(); !d
                .isAfter(to.atZone(ZoneOffset.UTC).toLocalDate()); d = d.plusDays(1)) {
            ...
            BigDecimal dayRevenue = orderRepository.sumTotalAmountByStatusInAndOrderDateBetween(completed, dayStart,
                    dayEnd);
            long dayCount = orderRepository.countByOrderDateBetween(dayStart, dayEnd);
            byDay.add(new RevenueStatsDto.RevenueByDayDto(d.toString(),
                    dayRevenue != null ? dayRevenue : BigDecimal.ZERO, dayCount));
        }
